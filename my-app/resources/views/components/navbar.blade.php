<div class="d-flex py-3 border-bottom">
    <ul class="nav nav-pills">
        <li>
            <i class="fas fa-cash-register"></i>
        </li>
        <li class="nav-item px-2">
            קופה
        </li>
    </ul>
    <ul class="nav nav-pills me-auto px-3">
        <li>
            <i data-bs-target="#sidebar" data-bs-toggle="collapse" class="cart-button fas fa-shopping-cart"
                style="cursor: pointer; display: none;"></i>
        </li>

    </ul>
</div>
<div class="d-flex py-3 border-bottom">
    <ul class="nav nav-pills">
        <li class="nav-item "><a id="mutzarim_text" class="nav_button text-decoration-none text-muted"
                href="/">מוצרים</a></li>
        <li class="nav-item px-4"><a id="favorites_text"class="nav_button text-decoration-none text-muted"
                href="/favorites_menu">מועדפים</a></li>
    </ul>
</div>
